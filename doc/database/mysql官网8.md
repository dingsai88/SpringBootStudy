...
作为DBA，您必须管理磁盘IO以防止IO子系统饱和，并管理磁盘空间以避免满存储设备。
ACID设计需要一定量的IO可能多余，但有助于数据的可靠性。


**15.11.1 InnoDB磁盘I / O**
InnoDB在可能的情况下使用异步磁盘IO，方法是创建多个线程来处理IO操作，同时允许其他数据库操作在IO
进行时候继续。在linux和windows平台上，innodb使用可用的OS和库功能来执行本机异步IO。
在其他平台上，innodb仍然使用IO线程，但是这些线程实际上可能会等待IO请求完成。

**预读Read-Ahead**
如果innodb可以确定很快有可能需要数据的可能性很大，它将执行预读操作将数据带入缓冲池，
以便在内存中可用。对连续数据发出一些大的预读请求可能比发出几个较小的，分散的
请求更为有效。有两种预读方式Innodb:

+ 在顺序预读中，对表空间中某个段访问时顺序的，它会将一批数据库页提前发布到IO系统
+ 在随机预读中，如果发现某些区域似乎正在被完全读入缓冲池，它将剩余的读操作发布到I / O系统



**双写缓冲区Doublewrite Buffer**
InnoDB使用一种新颖的文件刷新技术，该技术涉及一种称为doublewrite缓冲区的结构，该结构 在大多数情况下默认为启用（innodb_doublewrite=ON）。它为意外退出或停电后的恢复增加了安全性，并通过减少fsync()操作需求来提高了大多数Unix类型的性能。

在将页面InnoDB 写入数据文件之前，首先将它们写入称为doublewrite缓冲区的存储区域。仅在完成对双InnoDB写缓冲区的写入和刷新之后，才将页面写入数据文件中的相应位置。如果在页面写入过程中存在操作系统，存储子系统或意外的 mysqld进程退出（导致页面损坏的 情况），InnoDB则稍后可以在恢复期间从doublewrite缓冲区中找到该页面的良好副本。
...
在每页中容纳更多数据的一种方法是使用 压缩的行格式。对于使用BLOB或大文本字段的表， 紧凑的行格式允许将那些大列与该行的其余部分分开存储，从而减少了I / O开销和不引用这些列的查询的内存使用。

MySQL实例中的所有磁盘数据结构共享相同的页面大小。

**Extent区 1MB(64个page16K)**
https://dev.mysql.com/doc/refman/8.0/en/glossary.html#glos_extent
A group of pages within a tablespace.
对于默认的 页面大小16KB，一个扩展区包含64个页面。
在MySQL 5.6中，InnoDB实例的页面大小 可以是4KB，8KB或16KB
InnoDBMySQL 5.7.6中增加了 对32KB和64KB页面大小的支持。对于32KB页面大小，扩展区大小为2MB。对于64KB页面大小，扩展区大小为4MB。

**Segments段**
表空间中划分。如果表空间类似于目录，则段类似于该目录中的文件。

MySQL 8.0之前，系统表空间还包括一个或多个 用于 撤消日志的回滚段。
InnoDB给每个index索引分配了两个段segment: B树的非叶节点、叶节点
...
每个表空间由数据库页组成。mysql事例中每个表空间都具有相同的页面大小16K.
默认情况下，所有表空间的页面大小均为16KB;您可能指定大小innodb_page_size在创建
mysql示例时指定页面大小8K4K32K64K。

这些页page被分为大小为1MB的 extents区，用于最大16kb页(64个16K的页page；128个8Kpage;256个4K的页)
对于32K的页page,拓展图大小为2MB。对于64KB的页，拓展区大小为4MB。
在文件表空间内被称为段在innodb中
(这些段和回滚段不同，回滚段实际上包含许多表空间段。)

当段在表空间内增长时，innodb将前32页一次分配给它。之后innodb开始将整个拓展区分配给该段。
innodb一次最多可以向一个大段中添加四个拓展区，以确保数据的良好顺序。


innodb每个索引分配两个段segments.一个用于B树的非叶节点，一个用于叶结点。
将叶子节点保持在磁盘上连续可以实现更好的顺序IO操作。
因为这些叶子结点包含实际的表数据。

表空间中得某些页面包含其他页面的位图，因为表空间中某些拓展区innodb无法整体分配给段，
而只能分配给单个页面。
...
从表中删除数据时，将InnoDB 收缩相应的B树索引。释放的空间是否可供其他用户使用取决于删除模式是否将单个页面或扩展区释放到表空间中。保证删除表或从表中删除所有行都可以将空间释放给其他用户，但请记住，删除的行仅通过清除操作才能物理删除， 清除操作会在不再需要事务回滚或一致读取后的一段时间自动发生。



**页面如何与表格行相关**

对于4k 8k 16k 32k  innodb_page_size设置，最大行长度略小于数据库页面的一般。例如，
对于默认的16k 的页page大小，最大行长度略小于8k。对于64KB页，最大行长度略小于16K。

如果某行未超过最大行长，则所有行都将存储在本地页面内。如果某行超出最大行长，
则选择可变长度进行外部页出差，直到该行适合最大行长限制。
可变长度列的外部页外部存储因行格式而异。


**紧凑compact和冗余 redundant 行格式**
当将可变长度列选择用于外部页外存储时，InnoDB将前768个字节本地存储在该行中，并将其余768个字节外部存储在溢出页中。
每个此类列都有其自己的溢出页面列表。768字节的前缀附带一个20字节的值，该值存储列的真实长度，并指向溢出列表，该值的其余部分存储在溢出列表中。




**动态 dynamic 和压缩 compressed 行格式**
如果将可变长度列选择用于外部页外存储，则InnoDB在该行中本地存储一个20字节的指针，其余部分在外部存储到溢出页中。
...
**15.11.5使用TRUNCATE TABLE回收磁盘空间**
当回收操作系统的磁盘空间 截断的 InnoDB表，该表必须存放在自己的的.ibd文件。若要将表存储在其自己的.ibd 文件中，innodb_file_per_table必须在创建表时启用该表。此外，在被截断的表和其他表之间不能有 外键约束，否则 TRUNCATE TABLE操作将失败。但是，允许在同一表中的两列之间使用外键约束。

当表被截断时，将其删除并在新.ibd文件中重新创建 ，并将释放的空间返回给操作系统。这与截断InnoDB存储在 InnoDB 系统表空间 （在中创建的表innodb_file_per_table=OFF）中的表以及存储在共享通用表InnoDB空间中的表（在表被截断后只能使用释放的空间） 相反 。

截断表并将磁盘空间返回给操作系统的功能还意味着 物理备份可以更小。截断存储在系统表空间（在时创建的表 innodb_file_per_table=OFF）或常规表空间中的表，会将表空间中未使用的空间块留在表中。
...
II.LOCK=SHARED：
允许并发查询，但阻止DML。
例如，在数据仓库表上使用此子句，您可以在其中延迟数据加载操作直到DDL操作完成，但是查询不能长时间延迟。

II.LOCK=DEFAULT：
允许尽可能多的并发（并发查询，DML或两者）。省略该LOCK子句与指定相同LOCK=DEFAULT。
当您不希望DDL语句的默认锁定级别导致表的任何可用性问题时，请使用此子句。

II.LOCK=EXCLUSIVE：
阻止并发查询和DML。
如果主要的关注点是在最短的时间内完成DDL操作，并且不需要并发查询和DML访问，请使用此子句。如果服务器应该处于空闲状态，则也可以使用此子句，以避免意外的表访问。
...
在线DDL功能即时和就地更改表 以及并发DML提供支持。此功能的有点包括:
**优点**

1.在生产环境中提高了响应能力和可用性。在这种环境中，让一张表几分钟或几小时都不可用是不现实的。


2.可以使用LOCK子句在DDL操作期间调整性能与并发之间的平衡。


3.与表复制方法相比，磁盘空间使用量和I / O开销更少。



**您无需执行任何特殊操作即可启用在线DDL**
默认情况下，MySQL会在允许的情况下立即执行或在适当的位置执行操作，并尽可能减少锁定。


**ALTER TABLE tbl_name ADD PRIMARY KEY (column), ALGORITHM=INPLACE, LOCK=NONE;**


ALGORITHM:(INSTANT即时、INPLACE就地、COPY)
...
**阶段1：初始化**
在初始化阶段，服务器将考虑存储引擎功能，语句中指定的操作以及用户指定的ALGORITHM和LOCK 选项，以确定在操作期间允许多少并发 。在此阶段，将使用共享的可升级元数据锁来保护当前表定义。



**阶段2：执行**
在此阶段，准备并执行该语句。元数据锁是否升级到独占取决于初始化阶段评估的因素。如果需要独占元数据锁，则仅在语句准备期间进行短暂锁定。
...
15.15.1有关压缩的  INFORMATION_SCHEMA表
15.15.2  INFORMATION_SCHEMA事务和锁定信息
15.15.3  INFORMATION_SCHEMA架构对象表
15.15.4  INFORMATION_SCHEMA FULLTEXT索引表
15.15.5  INFORMATION_SCHEMA缓冲池表
15.15.6  INFORMATION_SCHEMA指标表
15.15.7  INFORMATION_SCHEMA临时表信息表
...
II.标准InnoDB监视器显示以下类型的信息：


II.该InnoDB锁监控打印附加锁信息作为标准的一部分 InnoDB监视器输出。
...
**热备份**
该mysqlbackup命令，MySQL企业备份组件的一部分，让您备份运行MySQL实例，包括InnoDB表，以最小的中断操作，同时生产数据库的一致性快照。当mysqlbackup复制 InnoDB表时，对InnoDB表的读写 可以继续。MySQL Enterprise Backup还可以创建压缩备份文件，并备份表和数据库的子集。结合MySQL二进制日志，用户可以执行时间点恢复。MySQL Enterprise Backup是MySQL Enterprise订阅的一部分。




**冷备份**
如果可以关闭MySQL服务器，则可以进行物理备份，其中包含用于InnoDB管理其表的所有文件 。使用以下过程：
对MySQL服务器 执行慢速关机，并确保其停止运行且没有错误。
将所有InnoDB数据文件（ibdata文件和 .ibd文件）复制到一个安全的地方。
将所有InnoDB日志文件（个ib_logfile文件）复制到一个安全的地方。
将my.cnf一个或多个配置文件复制到一个安全的地方


**使用mysqldump进行逻辑备份**


除了物理备份之外，建议您通过使用mysqldump转储表来定期创建逻辑备份 。二进制文件可能会在不通知您的情况下被损坏。转储的表存储在人类可读的文本文件中，因此发现表损坏变得更加容易。另外，由于格式更简单，因此严重数据损坏的可能性也较小。mysqldump 还提供了一个--single-transaction 选项，可以使快照一致而不会锁定其他客户端。请参见第7.3.1节“建立备份策略”。
...
对于InnoDB使用表DYNAMIC 或 COMPRESSED 行格式的表， 索引键前缀长度限制为3072字节 。

对于InnoDB使用REDUNDANT 或 COMPACT 行格式的表， 索引键前缀长度限制为767个字节 。

多列索引最多允许16列。超过限制将返回错误。

对于4KB，8KB，16KB和32KB页面大小，最大的行大小（不包括页面外存储的任何可变长度列）都小于页面的一半。例如，默认的最大innodb_page_size16KB行大小 约为8000个字节。但是，对于InnoDB 64KB的页面大小，最大行大小约为16000字节。LONGBLOB和 LONGTEXT 列必须小于4GB，并且总行大小（包括BLOB和 TEXT列）必须小于4GB。
...
字段不能叫 DB_ROW_ID， DB_TRX_ID，和 DB_ROLL_PTR

ROW_FORMAT=COMPRESSED 页面大小大于16KB不受支持。
...
Blackhole：Blackhole存储引擎可以接受但不存储数据，类似于Unix/dev/null设备。查询总是返回一个空集。
...
默认情况下，复制是异步的；副本无需永久连接即可从源接收更新。根据配置，您可以复制数据库中的所有数据库，选定的数据库，甚至选定的表。


**MySQL中复制的优点包括：**

横向扩展解决方案-在多个副本之间分散负载以提高性能。


数据安全性-由于副本可以暂停复制过程，因此可以在副本上运行备份服务而不会破坏相应的源数据。
...
基于语句的复制（SBR），它复制整个SQL语句  Statement Based Replication (SBR)；
基于行的复制（RBR），它仅复制更改的行  Row Based Replication (RBR)。
第三种混合混合复制（MBR） Mixed Based Replication (MBR).
...
**通用任务**
在源上，必须确保启用了二进制日志记录，并配置唯一的服务器ID。
在要连接到源的每个副本上，必须配置唯一的服务器ID。
（可选）为您的副本创建一个单独的用户，以便在读取二进制日志以进行复制时在与源进行身份验证期间使用。
在创建数据快照或开始复制过程之前，应在源上将当前位置记录在二进制日志中。配置副本时，您需要此信息，以便副本知道二进制日志中从何处开始执行事件。
存储引擎会影响创建快照的方式。MyISAM，必须在源上停止处理语句以获得读锁;InnoDB，则不需要读取锁定
使用连接到源的设置配置副本，例如主机名，登录凭据以及二进制日志文件名和位置。
根据您的系统，在源和副本上实施特定于复制的安全措施。
...
17.1.5.1配置多源复制
17.1.5.2为基于GTID的复制配置多源副本
17.1.5.3将基于GTID的源添加到多源副本中
17.1.5.4将基于二进制日志的复制源添加到多源副本中
17.1.5.5启动多源副本
17.1.5.6停止多源副本
17.1.5.7重置多源副本
17.1.5.8监视多源复制
...
**17.2复制实施 replication-implementation**
...
复制基于源服务器，在其二进制日志中跟踪其数据库的所有更改（更新，删除等）

17.2.1复制格式 RBR默认

II.statement-based binary logging (SBR)
记录sql语句


II.row-based logging  (RBR)  默认
基于行复制
...
**17.2.1.1基于语句的复制和基于行的复制的优缺点**
每种二进制日志记录格式都有优点和缺点。

对于大多数用户而言，混合复制格式应提供数据完整性和性能的最佳组合。


**基于语句的复制的优点 statement-based replication(SBR)**
+ 技术成熟 
+  更少的数据写入日志。   当更新或删除影响许多行时，这将导致 日志文件所需的存储空间大大减少。这也意味着可以更快地完成备份的备份和还原。
+ 日志文件包含了所有进行更改的语句，可以审核数据库。

**基于语句的缺点 statment-based replication(SBR)**
+ 依赖于不确定的存储返回的语句: UDF光盘或者其他存储，返回数据是存储返回的
+ delete update 使用不带limit 、order by 等不确定的返回的操作
+ 使用锁定读取的语句  for update、for share结果可能不一样
+ UDFs去中心化的存储 必须用在副本上。
+ 部分语句得到无法正确的复制 uuid()、rand()、sleep()、 NOW();
+ INSERT ... SELECT 与基于行的复制相比，需要更多的行级锁。
+ UPDATE WHERE与基于行的复制相比，需要进行表扫描的语句（因为在子句中未使用索引 ）必须锁定更多的行。
+ 对于InnoDB：使用的 INSERT语句将 AUTO_INCREMENT阻止其他不冲突的INSERT 语句。
+ 对于复杂的语句，在更新或插入行之前，必须在副本上评估并执行该语句。
+ 如果对副本的评估存在错误，尤其是在执行复杂的语句时，基于语句的复制可能会随着时间的流逝缓慢地增加受影响行上的错误余量。
+ 存储的函数以与NOW()调用语句相同的值执行 。
+ 表定义在源和副本上必须（几乎）相同。
...
**基于行的复制的优点 row-based replication (RBR)**
+ 可以复制所有更改。这是最安全的复制形式。
+ 对于以下类型的语句，源上需要较少的行锁，从而实现更高的并发性：
   + INSERT ... SELECT; INSERT 陈述与 AUTO_INCREMENT;UPDATE或 DELETE带有WHERE不使用键或不更改大多数已检查行的子句的语句 。
+  INSERT, UPDATE, or DELETE 行锁更少

**基于行的复制的缺点 row-based replication(RBR)**

+ RBR可以生成更多必须记录的数据。(改一个字段也要复制整行)行的复制将每个更改的行写入二进制日志。
+ 生成大BLOB值的确定性UDF 与基于语句的复制相比，使用基于行的复制所花费的时间更长。这
+ 您无法在副本上看到从源接收并执行了哪些语句。
+ 对于使用MyISAM 存储引擎的表,在副本上重放时需要更多锁定，row-based不支持插入并发




**17.2.1.2基于行的日志记录和复制的用法**
...
**基于语句记录日志SBL，基于行记录日志，混合记录日志。**
所使用的二进制日志的类型会影响日志的大小和效率。
因此，在基于行的复制（RBR）或基于语句的复制（SBR）之间进行选择取决于您的应用程序和环境。





**17.2.3复制线程replication**

**通过三个主要线程实现的:**  每个副本都是连接master 

二进制日志转储线程。  源创建一个线程，以便在副本连接时将二进制日志内容发送到副本。可

复制I / O线程。  在START REPLICA | SLAVE副本服务器上发出语句时，副本将创建一个I / O线程，该线程连接到源并要求其发送记录在其二进制日志中的更新。

复制SQL线程。  副本创建一个SQL线程以读取由复制I / O线程写入的中继日志，并执行其中包含的事务。
...
17.4.2处理副本意外停止
17.4.3监视基于行的复制
17.4.4将复制与不同的源存储和副本存储引擎一起使用
17.4.5使用复制进行横向扩展
17.4.6将不同的数据库复制到不同的副本
17.4.7提高复制性能
17.4.8故障转移期间切换源
17.4.9使用异步连接故障转移切换源
17.4.10半同步复制
...
**17.4.7提高复制性能**
随着连接到源的副本数量的增加，尽管每个负载使用最小的负载，但负载也增加了，因为每个副本都使用到源的客户端连接。

**复制除一个 source master  别的slave replica都从第二个源复制**

为此，必须按以下方式配置MySQL实例：

源1是将所有更改和更新都写入数据库的主要源。在两个源服务器上都启用了二进制日志记录，这是默认设置。

源2是服务器源1的副本，它为复制结构中的其余副本提供复制功能。Source 2是唯一允许连接Source 1的计算机。Source 2--log-slave-updates启用了该 选项（默认设置）。使用此选项，来自源1的复制指令也将写入源2的二进制日志中，以便随后可以将其复制到真实副本中。
