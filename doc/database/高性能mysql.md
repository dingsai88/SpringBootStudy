...
哈希索引
哈希索引 hashindex基于哈希表实现，只有精确匹配索引所有列的查询才有效。对
于每一行数据，存储引擎都会对所有的索引列计算一个哈希吗hashcode ,哈希吗是
一个较小的值，并且不同键值的行计算出来的哈希吗也不一样。哈希索引将所有的哈希吗
存储在索引中，同时在哈希表中保存指向每个数据和的指针。

在mysql中，只有memory引擎显示支持哈希索引。这也是memory引擎表的默认索引
类型，,memory引擎同事也支持btree索引。值得一提的是，memory引擎是支持非
...
假设索引使用假想的哈希函数，它返回下面的值（都是示例数据，非真实数据）
